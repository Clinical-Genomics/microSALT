...
if preset_config != '':
  try:
    ...
    preset_config['folders']['expec'] = os.path.abspath(os.path.join(pathlib.Path(__file__).parent.parent, 'unique_references/ExPEC.fsa'))
    #Check if release install exists
    for entry in os.listdir(get_python_lib()):
      if 'microSALT-' in entry:
        preset_config['folders']['expec'] = os.path.abspath(os.path.join(os.path.expandvars('$CONDA_PREFIX'), 'expec/ExPEC.fsa'))
        break
    preset_config['folders']['adapters'] = os.path.abspath(os.path.join(os.path.expandvars('$CONDA_PREFIX'), 'share/trimmomatic-0.39-1/adapters/'))
    ...
    for entry in preset_config.keys():
      if entry != '_comment':
        if isinstance(preset_config[entry], str) and '/' in preset_config[entry] and entry not in ['genologics']:
        ...
        #level two
          ...
          for thing in preset_config[entry].keys():
            if isinstance(preset_config[entry][thing], str) and '/' in preset_config[entry][thing] and entry not in ['genologics']:
