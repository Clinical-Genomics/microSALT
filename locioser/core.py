"""This initial script wraps assembly and loci calling into a single file
   Heavy WIP
...
#Create assembly sbatch job
#Wait to finish
#Create BLAST sbatch job
#Wait to finish
...
def main(indir):
  ...
  batchfile.write("makeblastdb -in {}/{} -dbtype nucl -parse_seqids -out {}\n".format(config["folders"]["references"], config["organism"], config["organism"]))
  #create run
  batchfile.write("blastn -db {}/{}.xmfa -query {}/assembly/contigs.fasta -out {}/search_loci\n\n".format(config["folders"]["references"], config["organism"], outdir, outdir))
