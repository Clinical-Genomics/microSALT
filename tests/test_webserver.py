...
def appscript():
    script = os.path.abspath(
        os.path.join(pathlib.Path(__file__).parent.parent, "microSALT/server/app.py")
    )
    return script
...
def test_pages(report_obj, mock_db):
    ...
    time.sleep(0.15)
    e = requests.get(
        "http://127.0.0.1:5000/microSALT/AAA1234/typing/all", allow_redirects=True
    )
    ...
    time.sleep(0.15)
    f = requests.get(
        "http://127.0.0.1:5000/microSALT/AAA1234/typing/escherichia_coli",
    ...
    time.sleep(0.15)
    g = requests.get(
        "http://127.0.0.1:5000/microSALT/STtracker/all", allow_redirects=True
    )
    assert g.status_code in [200, 500]

    time.sleep(0.15)
    h = requests.get(
        "http://127.0.0.1:5000/microSALT/STtracker/cust000", allow_redirects=True
    )
